...
export default function InsightsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/insights/today?userId=user-1`)
      .then(r => r.json())
  ...
  const hasData = !!(data?.today?.mood || data?.today?.stress || data?.today?.sleep);
          ...
          {(!data?.insights || data.insights.length === 0) ? (
            ...
            <div className="flex flex-col gap-2.5">
              {data.insights.map((insight, i) => {
                // Backend provides actionLink directly; fall back to null for non-navigable
